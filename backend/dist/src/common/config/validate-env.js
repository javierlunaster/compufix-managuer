...
function validateEnv() {
    ...
    if (missing.length > 0) {
        throw new Error(`Faltan variables de entorno obligatorias: ${missing.join(", ")}. Revisa tu archivo .env (compáralo con .env.example).`);
    }
    const isProduction = process.env.NODE_ENV === "production";
    if (isProduction) {
        const usingPlaceholder = REQUIRED_VARS.filter((key) => process.env[key] && PLACEHOLDER_VALUES.has(process.env[key]));
        if (usingPlaceholder.length > 0) {
            throw new Error(`No se puede arrancar en producción con valores de ejemplo sin cambiar: ${usingPlaceholder.join(", ")}. Genera secretos propios antes de desplegar.`);
        }
    }
}
