...
function isWinAnsiSafe(codePoint) {
    if (codePoint <= 0x7f)
        return true; // ASCII
    if (codePoint >= 0xa0 && codePoint <= 0xff)
        return true; // Latin-1: tildes, ñ, ¿¡, etc.
...
class PdfBuilder {
    constructor() {
        this.doc = new pdfkit_1.default({ margin: 50, size: "letter", bufferPages: true });
    }
    header(params) {
        this.doc
            .fontSize(9)
    ...
    table(headers, rows, columnWidths) {
        ...
        for (const row of safeRows) {
            ...
            // la fila siguiente.
            const rowHeight = Math.max(...row.map((cell, i) => this.doc.heightOfString(cell, { width: widths[i] })));
